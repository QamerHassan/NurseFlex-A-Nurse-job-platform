      ...
      <div className="grid gap-8">
        {filteredShifts.length > 0 ? (
          filteredShifts.map((shift) => (
            <Card key={shift.id} className="group border-none shadow-sm hover:shadow-2xl hover:shadow-blue-100/50 bg-white rounded-[3rem] p-10 transition-all duration-500 relative overflow-hidden">
                <div className="absolute top-0 right-0 w-64 h-64 bg-slate-50 rounded-full blur-[100px] -mr-32 -mt-32 group-hover:bg-blue-50 transition-all duration-700"></div>
                
                <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-12 relative z-10">
                    <div className="space-y-6 flex-1">
                        <div className="flex items-center gap-4">
                            <h3 className="text-3xl font-bold text-slate-900 tracking-tight transition-colors group-hover:text-blue-600 leading-none">{shift.title}</h3>
                        ...
                        <div className="flex flex-wrap items-center gap-8">
                            <div className="flex items-center gap-3">
                                <div className="w-8 h-8 bg-slate-50 rounded-lg flex items-center justify-center text-slate-400 group-hover:bg-blue-50 group-hover:text-blue-400 transition-colors">
                                    <MapPin size={14} />
                                </div>
                            ...
                            <div className="flex items-center gap-3">
                                ...
                                <span className="text-xs font-bold text-slate-500">
                                    Posted: {mounted ? new Date(shift.createdAt).toLocaleDateString() : '...'}
                                </span>
                            </div>
                        </div>
                    </div>
                    ...
                    <div className="flex items-center gap-12 lg:pl-12 lg:border-l border-slate-100">
                        ...
                        <div className="flex items-center gap-4">
                            <Button asChild size="lg" className="h-16 px-10 rounded-2xl bg-blue-600 hover:bg-blue-700 font-bold text-base shadow-xl transition-all shadow-blue-100/50">
                            ...
                            <DropdownMenu>
                                ...
                                <DropdownMenuContent align="end" className="w-56 p-2 rounded-2xl border-slate-100 shadow-2xl">
                                    ...
                                    <DropdownMenuItem className="p-3 font-bold text-sm rounded-xl cursor-pointer">
                                        <Edit3 className="mr-3 w-4 h-4 text-blue-500" /> Edit Job
                                    </DropdownMenuItem>
                                    <DropdownMenuItem className="p-3 font-bold text-sm rounded-xl cursor-pointer">
                                        <Copy className="mr-3 w-4 h-4 text-blue-500" /> Duplicate Job
                                    </DropdownMenuItem>
                                    <DropdownMenuItem className="p-3 font-bold text-sm rounded-xl cursor-pointer">
                                        <Archive className="mr-3 w-4 h-4 text-blue-500" /> Archive Job
                                    </DropdownMenuItem>
                                    <DropdownMenuSeparator />
                                    <DropdownMenuItem 
          ...
          <Card className="flex flex-col items-center justify-center py-32 text-center bg-white rounded-[3rem] border border-dashed border-slate-200 shadow-inner overflow-hidden relative">
            ...
            <Button asChild size="lg" className="h-16 px-12 rounded-2xl bg-slate-900 hover:bg-blue-600 font-bold text-lg shadow-2xl shadow-slate-200 transition-all active:scale-[0.98]">
