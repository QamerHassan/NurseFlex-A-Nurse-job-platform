...
export default function BusinessMessagesPage() {
  ...
  return (
    <div className="h-[calc(100vh-160px)] flex bg-white/50 rounded-[3rem] overflow-hidden border border-slate-100 shadow-2xl shadow-blue-50/50 animate-in fade-in duration-700">
      ...
      <aside className="w-96 bg-white border-r border-slate-50 flex flex-col relative z-20">
        <header className="p-8 pb-6 border-b border-slate-50">
            <div className="flex items-center justify-between mb-8">
                ...
                <Badge variant="outline" className="h-6 px-3 rounded-full border-blue-100 text-blue-600 bg-blue-50/50 font-bold text-[9px]">Live Chat</Badge>
            </div>
                ...
                {conversations.length > 0 ? conversations.map((conv) => {
                    ...
                    return (
                        <button
                            ...
                            className={`w-full p-6 text-left flex gap-4 group transition-all duration-300 ${isSelected ? 'bg-white shadow-2xl shadow-blue-50 relative z-10 scale-[1.02]' : 'hover:bg-slate-50/50'}`}
                        >
                            <div className="relative">
                                <Avatar className={`h-14 w-14 rounded-2xl shadow-lg transition-transform group-hover:scale-105 duration-500 ring-2 ${isSelected ? 'ring-blue-500/20' : 'ring-transparent'}`}>
                                    <AvatarFallback className={`font-bold text-xl ${isSelected ? 'bg-slate-900 text-white' : 'bg-blue-50 text-blue-600 shadow-inner'}`}>
                            ...
                            <div className="flex-1 min-w-0 pt-1">
                                <div className="flex justify-between items-start mb-1">
                                    <span className={`font-bold truncate text-[13px] tracking-tight ${isSelected ? 'text-blue-600' : 'text-slate-900'}`}>{other?.profile?.name || other?.email.split('@')[0]}</span>
                            ...
                            {isSelected && (
                                <div className="absolute left-0 top-1/2 -translate-y-1/2 w-1.5 h-8 bg-blue-600 rounded-r-full shadow-2xl shadow-blue-500/50"></div>
                            )}
                        </button>
                    );
                }) : (
      ...
      <main className="flex-1 bg-white flex flex-col relative overflow-hidden">
        {selectedConv ? (
          <>
            ...
            <header className="h-[88px] px-10 border-b border-slate-50 flex items-center justify-between sticky top-0 z-10 bg-white/80 backdrop-blur-md">
                <div className="flex items-center gap-6">
                    <Avatar className="h-12 w-12 rounded-2xl shadow-2xl shadow-blue-100 ring-2 ring-white">
                ...
                <div className="flex items-center gap-3">
                    <Button variant="ghost" size="icon" className="h-12 w-12 rounded-2xl text-slate-400 hover:text-blue-600 hover:bg-blue-50">
                        <Phone size={20} />
                    </Button>
                    <Button variant="ghost" size="icon" className="h-12 w-12 rounded-2xl text-slate-400 hover:text-blue-600 hover:bg-blue-50">
                        <Video size={20} />
                ...
                <div className="space-y-12 pb-10">
                    ...
                    {msgLoading ? (
                        ...
                        <div className="flex flex-col items-center justify-center py-32 text-center">
                            <div className="w-24 h-24 bg-white rounded-[32px] flex items-center justify-center shadow-2xl shadow-blue-50/50 mb-8 border border-slate-50">
                                <Zap size={40} className="text-blue-500 fill-blue-500 opacity-20" />
                            </div>
            ...
            <footer className="p-8 bg-white border-t border-slate-50">
              <form onSubmit={handleSendMessage} className="bg-slate-50 rounded-[2.5rem] p-2 flex items-center gap-3 border border-slate-100 focus-within:ring-8 focus-within:ring-blue-50/50 focus-within:bg-white focus-within:border-blue-600 transition-all duration-500">
                  <Button type="button" variant="ghost" size="icon" className="h-12 w-12 rounded-full text-slate-400 hover:text-blue-600 hover:bg-white shrink-0">
                      <Paperclip size={20} />
                  </Button>
                  <Input
                      value={newMessage}
                  ...
                  <Button 
                      ...
                      className="h-12 w-12 rounded-full bg-slate-900 hover:bg-blue-600 text-white shadow-2xl transition-all duration-300 shrink-0"
                  >
          ...
          <div className="flex-1 flex flex-col items-center justify-center p-20 bg-[radial-gradient(#e2e8f0_1px,transparent_1px)] [background-size:32px_32px]">
            <Card className="max-w-md w-full border-none shadow-2xl shadow-blue-50/50 rounded-[3rem] p-12 text-center bg-white/80 backdrop-blur-xl">
                <div className="w-24 h-24 bg-slate-900 rounded-[40px] flex items-center justify-center text-white shadow-2xl shadow-blue-100 mx-auto mb-10 transition-transform hover:scale-105 duration-500">
