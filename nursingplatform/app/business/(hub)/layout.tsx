...
export default function BusinessLayout({ children }: { children: React.ReactNode }) {
  ...
  const [businessName, setBusinessName] = useState('Employer Hub');
  ...
  useEffect(() => {
    const checkAuth = async () => {
      ...
      if (!token && !session) {
        router.push('/auth/login?portal=business');
        return;
      }
      ...
      if (!user && session?.user) {
        user = { id: (session.user as any).id, role: (session.user as any).role || 'BUSINESS', status: (session.user as any).status || 'PENDING' };
        localStorage.setItem('business_user', JSON.stringify(user));
      }

      if (!user?.id) { router.push('/auth/login?portal=business'); return; }

      try {
        if (user.role !== 'BUSINESS') { router.push('/auth/login?portal=business'); return; }
        ...
        localStorage.setItem('business_user', JSON.stringify(latestUser));
        setBusinessName(latestUser.name || 'Employer Hub');

        if (latestUser.status === 'PENDING') {
          router.push(subscription ? '/auth/pending' : '/business/payment?tierId=basic');
          return;
        }
        if (latestUser.status === 'REJECTED') { router.push('/auth/pending'); return; }
        if (!subscription) { router.push('/business/payment?tierId=basic'); return; }
  ...
  if (isAuthorized === null) {
    return (
      <div className="min-h-screen flex flex-col items-center justify-center bg-white space-y-4">
        <div className="w-12 h-12 bg-[#ec4899] rounded-xl flex items-center justify-center text-white">
            <PlusCircle size={24} />
        </div>
        <div className="flex flex-col items-center gap-2">
          <Loader2 className="animate-spin text-[#ec4899]" size={20} />
          <p className="text-xs font-semibold text-slate-400">Loading your hub...</p>
        </div>
      </div>
    );
  }
    ...
    <div className="flex flex-col h-full bg-white text-slate-500 overflow-hidden w-full">
      ...
      <div className={`p-6 border-b border-slate-100 flex items-center ${!isExpanded ? 'justify-center' : 'justify-between'}`}>
        <Link href="/business/dashboard" className="flex items-center gap-3 group">
            <div className="w-10 h-10 shrink-0 bg-[#ec4899] rounded-lg flex items-center justify-center text-white font-bold">
                N
            </div>
            {isExpanded && (
                <div className="whitespace-nowrap animate-in fade-in slide-in-from-left-2">
                    <h2 className="text-lg font-bold text-[#ec4899] tracking-tight">Employer Hub</h2>
                </div>
            )}
        </Link>
        {isExpanded && showPin && (
            <Button
                ...
                className="h-8 w-8 text-slate-400 hover:text-[#ec4899] transition-all rounded-md"
            >
                    ...
                    {NAV_ITEMS.map((item) => {
                        ...
                        return (
                            <Link key={item.href} href={item.href}>
                                <div className={`flex items-center gap-3 h-11 ${!isExpanded ? 'px-0 justify-center' : 'px-3'} rounded-lg transition-all duration-200 group ${active ? 'bg-pink-50 text-[#ec4899] border-l-4 border-[#ec4899] rounded-l-none' : 'hover:bg-slate-50 hover:text-slate-900'}`}>
                                    <item.icon size={18} className={`${active ? 'text-[#ec4899]' : 'text-slate-400 group-hover:text-slate-600'} transition-colors shrink-0`} />
      ...
      <div className="p-4 border-t border-slate-100">
        <div className={`flex items-center ${!isExpanded ? 'justify-center font-bold' : 'gap-3'} transition-all`}>
            <Avatar className="h-8 w-8 shrink-0 rounded-md">
                <AvatarFallback className="bg-pink-100 text-[#ec4899] text-xs font-bold">{businessName[0]}</AvatarFallback>
            </Avatar>
            {isExpanded && (
      ...
      <div className="flex-1 flex flex-col lg:ml-[4.5rem] transition-all duration-300 ease-in-out">
        <header className="h-16 px-6 bg-white border-b border-slate-200 flex items-center justify-between sticky top-0 z-40">
            <div className="flex items-center gap-4 lg:hidden">
                ...
                <h1 className="text-sm font-bold text-[#ec4899]">Employer Hub</h1>
            </div>
