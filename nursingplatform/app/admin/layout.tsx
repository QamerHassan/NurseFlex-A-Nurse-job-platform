            ...
            <CardContent className="p-0 space-y-6">
              ...
              <form onSubmit={handleLogin} className="space-y-4">
                <div className="space-y-2">
                  ...
                  <div className="relative">
                    ...
                    <input
                      ...
                      className="w-full pl-12 pr-12 h-12 bg-white border border-slate-200 rounded-xl text-slate-900 outline-none focus:border-blue-500 focus:ring-2 focus:ring-blue-500/10 transition-all font-mono"
                    />
                    <button
                      type="button"
                ...
                <Button
                  type="submit"
                  disabled={loading}
                  className="w-full h-12 bg-blue-600 hover:bg-blue-700 text-white rounded-xl font-bold text-sm shadow-sm transition-all active:scale-95 disabled:opacity-50"
                >
        ...
        <div className="p-5 border-b border-slate-100 flex items-center justify-between min-h-[64px]">
          {isExpanded && (
            <div className="flex items-center gap-3 animate-in slide-in-from-left-4">
              <div className="w-8 h-8 bg-blue-600 rounded-lg flex items-center justify-center text-white shrink-0 font-bold">
                A
              </div>
              <div>
                <h2 className="text-sm font-bold tracking-tight text-slate-900">Admin</h2>
                <p className="text-[10px] font-semibold text-green-600 leading-none">Management</p>
              </div>
            </div>
          )}
          <Button
            ...
            className="h-8 w-8 text-slate-400 hover:text-blue-600 rounded-md transition-all"
          >
              ...
              {MANAGEMENT_LINKS.map(({ href, label, icon: Icon }) => {
                ...
                return (
                  <Link key={href} href={href}
                    className={`flex items-center gap-3 px-3 py-3 rounded-lg transition-all duration-200 group ${active ? 'bg-blue-50 text-blue-600 border-l-4 border-blue-600 rounded-l-none' : 'text-slate-500 hover:text-slate-900 hover:bg-slate-50'}`}>
                    <Icon size={18} className={`${active ? 'text-blue-600' : 'text-slate-400 group-hover:text-slate-600'} transition-colors shrink-0`} />
          ...
          <div className="space-y-3">
            ...
            <div className="space-y-1">
              {TOOLS_LINKS.map(({ href, label, icon: Icon }) => {
                ...
                return (
                  <Link key={href} href={href}
                    className={`flex items-center gap-3 px-3 py-3 rounded-lg transition-all duration-200 group ${active ? 'bg-green-50 text-green-600 border-l-4 border-green-600 rounded-l-none' : 'text-slate-500 hover:text-slate-900 hover:bg-slate-50'}`}>
                    <Icon size={18} className={`${active ? 'text-green-600' : 'text-slate-400 group-hover:text-slate-600'} transition-colors shrink-0`} />
                    {isExpanded && <span className="text-sm font-semibold tracking-tight">{label}</span>}
                  </Link>
                );
              })}
      ...
      <div className="flex-1 flex flex-col min-h-screen">
        ...
        <header className="h-16 bg-white border-b border-slate-200 flex items-center justify-between px-6 md:px-10 sticky top-0 z-[50]">
          <div className="flex items-center gap-4">
            <div className="flex flex-col">
              ...
              <p className="text-[10px] font-semibold text-green-600">Admin Portal</p>
            </div>
          </div>
        ...
        <footer className="h-12 border-t border-slate-200 px-10 flex items-center justify-between text-[10px] font-bold uppercase tracking-widest text-slate-400">
          ...
          <div className="flex gap-6">
            <Link href="#" className="hover:text-blue-600 transition-colors">Logs</Link>
            <Link href="#" className="hover:text-green-600 transition-colors">Security</Link>
          </div>
        </footer>
      </div>
    </div>
