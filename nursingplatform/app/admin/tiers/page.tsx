    ...
    <div className="space-y-10 animate-in fade-in duration-700">
      ...
      <header className="flex flex-col md:flex-row justify-between items-start md:items-center gap-6">
        ...
        <div className="flex items-center gap-4">
          ...
          <Button
            onClick={addTier}
            className="h-12 px-6 rounded-xl bg-blue-600 hover:bg-blue-700 text-white font-bold text-sm gap-2 transition-all active:scale-95"
          >
      ...
      <div className="grid grid-cols-1 gap-6">
        {tiers.length > 0 ? (
          tiers.map((tier) => (
            <Card
              key={tier.id}
              className={`border border-slate-100 shadow-sm rounded-2xl overflow-hidden bg-white transition-all duration-300 hover:shadow-md ${tier.isPopular ? 'ring-2 ring-green-500/20' : ''}`}
            >
              <CardContent className="p-8 flex flex-col xl:flex-row gap-8 items-start xl:items-center">
                {/* Identifier Block */}
                <div className="flex-1 w-full flex items-center gap-4">
                  <div className={`w-12 h-12 rounded-xl flex items-center justify-center ${tier.isPopular ? 'bg-green-600 text-white' : 'bg-slate-900 text-white'}`}>
                ...
                <div className="flex gap-2 w-full xl:w-auto">
                  <Button
                    ...
                    className="h-12 flex-1 xl:w-40 rounded-xl bg-blue-600 hover:bg-blue-700 text-white font-bold text-sm shadow-xl shadow-blue-100 gap-2 transition-all active:scale-95"
                  >
          ...
          <div className="p-20 text-center space-y-4 border-2 border-dashed border-slate-100 rounded-3xl">
            ...
            <Button onClick={addTier} variant="outline" className="text-blue-600 border-blue-100 hover:bg-blue-50 rounded-xl">Create My First Plan</Button>
          </div>
        )}
      </div>
