    ...
    >
      <div className="max-w-7xl mx-auto px-6 flex items-center justify-between">
        ...
        <Link href="/" className="flex items-center gap-3 group">
          <div className="w-10 h-10 bg-blue-600 rounded-2xl flex items-center justify-center text-white shadow-2xl shadow-blue-200 group-hover:scale-105 transition-transform duration-500">
        ...
        <nav className="hidden lg:flex items-center gap-1">
          {navLinks.map((link) => (
            <Button
              ...
              className={`text-sm font-bold tracking-tight rounded-xl px-4 ${
                pathname === link.href ? 'text-blue-600 bg-blue-50/50' : 'text-slate-500 hover:text-blue-600 hover:bg-blue-50/30'
              }`}
            >
          ...
          {role === 'BUSINESS' ? (
            <Button asChild className="h-10 px-6 rounded-xl font-black uppercase tracking-tighter italic bg-blue-600 group shadow-md shadow-blue-200 hover:bg-blue-700 transition-all text-white">
               <Link href="/business/dashboard">Business Hub</Link>
            </Button>
          ) : (
            <Button variant="outline" asChild className="h-10 px-6 rounded-xl font-bold border-blue-100 text-blue-600 hover:bg-blue-50">
        ...
        <div className="hidden lg:flex items-center gap-4">
          {mounted && (isLoggedIn ? (
            <div className="flex items-center gap-4">
              <Button variant="ghost" size="icon" asChild className="h-10 w-10 rounded-xl text-slate-400 hover:text-blue-600 hover:bg-blue-50 transition-colors">
            ...
            <Button variant="ghost" asChild className="h-11 px-6 rounded-xl font-bold text-blue-600 hover:bg-blue-50">
                <Link href="/auth/login">Sign In</Link>
              </Button>
              <Button asChild className="h-11 px-7 rounded-2xl font-black uppercase tracking-tighter italic bg-blue-600 shadow-md shadow-blue-200 hover:bg-blue-700 transition-all active:scale-[0.98] text-white">
                    ...
                    <SheetContent className="w-full sm:max-w-xs p-8 rounded-l-3xl border-none shadow-2xl">
                        ...
                        <nav className="flex flex-col gap-2">
                            ...
                            <Button variant="ghost" asChild className="justify-start h-14 px-4 rounded-xl font-bold text-lg text-slate-600">
                                <Link href="/business/login">Business Portal</Link>
                            </Button>
                        </nav>
                        <div className="mt-10 pt-10 border-t border-slate-50 space-y-4">
                            {isLoggedIn ? (
                                <Button asChild size="lg" className="w-full h-16 rounded-2xl font-black uppercase tracking-tighter italic bg-blue-600 shadow-xl shadow-blue-100 hover:bg-blue-700">
